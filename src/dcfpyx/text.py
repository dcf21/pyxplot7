    ...
    def execute(self, expr, texmessages):
        ...
        if not self.texruns:
            ...
            if self.texipc:
                ...
                ipcflag = ""
            sp = subprocess.Popen("%s%s %s" % (self.mode, ipcflag, self.texfilename), universal_newlines=True, shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
            self.texinput  = sp.stdin
            self.texoutput = sp.stdout
