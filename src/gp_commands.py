...
eps@2:directive = < %q:filename | %S:filename > ( at@2 %f:x ,@n %f:y ~ rotate@1 %f:rotation ~ width@1 %f:width ~ height@1 %f:height )
exit@2:directive:quit =
fit@3:directive = [ \[@n { { %f:min } < :@n | to@n > { %f:max } } \]@n ]:range_list %v:fit_function \(@n [ %v:inputvar ]:operands, \)@n < %q:filename | %S:filename > ( every@1 [ { %d:every_item } ]:every_list: ~ index@1 %f:index ~ select@1 %E:select_criterion ~ using@1 { < rows@1:use_rows | columns@1:use_columns > } [ %E:using_item ]:using_list: ) via@1 [ %v:fit_variable ]:fit_variables,
...
jpeg@1:directive = < %q:filename | %S:filename > ( at@2 %f:x ,@n %f:y ~ rotate@1 %f:rotation ~ width@1 %f:width ~ height@1 %f:height )
...
set@2:directive %a:axis label@1:set_option:xlabel = %q:label_text
set@2:directive %a:axis range@1:set_option = \[@n { %f:min } < :@n | to@n > { %f:max } \]@n
...
set@2:directive key@1:set_option = < below@2:pos | outside@1:pos | ( < left@1:xpos | right@1:xpos | xcentre@1:xpos | xcenter@1:xpos:xcentre > ~ < top@1:ypos | bottom@2:ypos | ycentre@1:ypos | ycenter@1:ypos:ycentre > ) > { %f:x_offset ,@n %f:y_offset }
set@2:directive keycolumns@4:set_option = %d:key_columns
set@2:directive label@2:set_option = %d:label_id %q:label_text { at@1 } { < first@1:x_system | second@1:x_system | screen@2:x_system | graph@1:x_system | axis@n %d:x_system > } %f:x_position ,@n { < first@1:y_system | second@1:y_system | screen@2:y_system | graph@1:y_system | axis@n %d:y_system > } %f:y_position { rotate@1 %f:rotation }
...
set@2:directive output@1:set_option = < %q:filename | %S:filename >
set@2:directive palette@1:set_option = [ %s:colour ]:palette,
set@2:directive papersize@3:set_option = < %f:x_size ,@n %f:y_size | %s:paper_name | %q:paper_name >
...
set@2:directive terminal@1:set_option = ( < x11_singlewindow@1:term:X11_singlewindow | x11_multiwindow@5:term:X11_multiwindow | x11_persist@5:term:X11_persist | postscript@1:term:PS | ps@2:term:PS | eps@1:term:EPS | pdf@2:term:PDF | png@2:term:PNG | gif@1:term:GIF | jpg@1:term:JPG | jpeg@1:term:JPG > ~ < colour@1:col:ON | color@1:col:ON | monochrome@1:col:OFF > ~ < enlarge@1:enlarge:ON | noenlarge@3:enlarge:OFF > ~ < landscape@1:land:ON | portrait@2:land:OFF > ~ < transparent@1:trans:ON | solid@1:trans:OFF > ~ < invert@1:invert:ON | noinvert@1:invert:OFF > )
...
set@2:directive title@2:set_option = %q:title { %f:x_offset ,@n %f:y_offset }
...
spline@3:directive = [ \[@n { { %f:min } < :@n | to@n > { %f:max } } \]@n ]:range_list %v:fit_function \()@2 < %q:filename | %S:filename > ( every@1 [ { %d:every_item } ]:every_list: ~ index@1 %f:index ~ select@1 %E:select_criterion ~ smooth@1 %f:smooth ~ using@1 { < rows@1:use_rows | columns@1:use_columns > } [ %E:using_item ]:using_list: )
tabulate@2:directive = [ \[@n { { %f:min } < :@n | to@n > { %f:max } } \]@n ]:range_list < %q:filename | [ %e:expression ]:expression_list: > ( every@1 [ { %d:every_item } ]:every_list: ~ index@1 %f:index ~ select@1 %E:select_criterion ~ using@1 { < rows@1:use_rows | columns@1:use_columns > } [ %E:using_item ]:using_list: )
histogram@2:directive = 
