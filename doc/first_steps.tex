...
The simplest way to start PyXPlot is to type ``\texttt{pyxplot}'' at a
...
commences after the execution of \texttt{foo1.ppl}, and should be terminated in
the usual way after use, with the ``\texttt{exit}'' or ``\texttt{quit}''
commands.  Afterwards, the command script \texttt{foo2.ppl} would execute.
...
file \texttt{foo.ppl} into the present session.

Usually a text editor is used to produce PyXPlot command scripts, but the
\texttt{save}\index{save command@\texttt{save} command} command may also
assist. Thish stores a history of the commands executed in the present
...
following would plot data from a file `\texttt{data.dat}', taking the
...
format\footnote{If the filename of a datafile ends with a \texttt{.gz} suffix,
...
compatible with the Comma Seperated Values (CSV) format produced by many
applications, including Microsoft Excel.}\index{csv files}\index{spreadsheets,
importing data from}\index{Microsoft Excel}:
...
PyXPlot has a {\tt print} command\index{print command@\texttt{print} command}
for displaying strings and the results of calculations to the terminal, for
example:
...
\noindent Values may also be substituted into strings using the \% operator,
which works in a similar fashion to python string substitution. The list of
values to be substituted into the string should be a ()-bracketed list:
...
quote characters -- for example, to put a German umlaut on the name ``J\"org''
in the \LaTeX\ string ``\texttt{J$\backslash$"org's Data}'' -- PyXPlot
...
PyXPlot considers $\backslash$ an escape character. To render the example
string above, one would type:
...
escape character, used so that the " character is not interpretted as
...
followed by almost any word that can follow the \texttt{set} command, such as
\texttt{xlabel} or \texttt{title}, to return that setting to its default
...
recognised by PyXPlot is given in Table~\ref{operators_table}.
...
\texttt{data.dat} on the vertical axis, against the the contents of the third
...
within each index. To do so, the \texttt{every} modifier is used, which takes
up to six values, separated by colons:\index{every modifier@\texttt{every}
modifier}
...
the syntax of the \texttt{set terminal} command in PyXPlot is somewhat
different from that which they are used to; see Section~\ref{set_terminal2}.}
is used to specify the output format that is required, and the \texttt{set
output} command\index{set output command@\texttt{set output} command} is used
to specify the file to which output should be directed. For example,
...
The \texttt{set terminal} command can also be used to configure various output
options within each supported file format.  For example, the following commands
would produce black-and-white or colour output respectively:
...
produces files in the portable document format (pdf) read by Adobe Acrobat:
...
gif and png images with transparent backgrounds, see
Section~\ref{set_terminal2}.
...
are added to the pre-existing plot.
...
command@\texttt{set style} command} command changes the default style used when
...
\noindent A similar effect may be obtained using the \texttt{set
autoscale}\index{set autoscale command@\texttt{set autoscale} command} command,
...
Axes can be set to have logarithmic scales by using the \texttt{set
...
the \texttt{fit} command. A simple example might be:\footnote{In gnuplot, this
example would have been written \texttt{fit f(x) ...}, rather than \texttt{fit
...
process are listed after the keyword ``\texttt{via}'' in the {\tt fit} command.
The keywords \texttt{index}, \texttt{every} and \texttt{using} have the same
meanings here as in the plot command.\footnote{The \texttt{select} keyword, to
be introduced in Section~\ref{select_modifier} can also be used.}
...
within PyXPlot by prefixing them with an {\tt !} character\index{!
command@\texttt{!} command}. The remainder of the line is sent directly to the
shell, for example:
...
PyXPlot has its own {\tt cd}\index{cd command@\texttt{cd} command} command for
this purpose, as well as its own pwd\index{pwd command@\texttt{pwd} command}:
...
surface plotting -- i.e. the {\tt splot}\index{splot command@\texttt{splot}
command} of gnuplot. It also does not support the plotting of parametric
functions.
