...
The {\tt gif}, {\tt png} and {\tt jpg} terminals result in some loss
...
using the command {\tt set dpi <value>}. Alternatively, it may be changed
using the {\tt DPI} option in the {\tt settings} section of a
configuration file (see Section~\ref{configuration}).\index{set dpi
command@{\tt set dpi} command}\index{resolution of bitmap
...
locale settings. It may be changed, however, with the {\tt set
papersize}\index{set papersize command@{\tt set papersize} command} command,
which may be followed either by the name of a recognised paper size, or by the
dimensions of a user-defined size, specified as a {\tt height},
...
\hline
\end{tabular}
\caption{A list of all of the named paper sizes recognised by the {\tt set papersize}\index{set
papersize command@{\tt set papersize} command} command, with their heights, $h$, and widths, $w$, measured in millimetres.}
\label{paper_sizes}
\end{figure}
...
{\tt script}. One difference, however, is that if multiple script files are
...
example, suppose that a script {\tt foo} produces postscript output
{\tt foo.ps}. The following two commands could be used to give a live view
