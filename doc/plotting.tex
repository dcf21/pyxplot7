...
widths equalling that specified in the \texttt{set boxwidth}\index{set boxwidth
command@\texttt{set boxwidth} command} command.  Consequently, there may be
...
behaviour, the \texttt{wboxes}\index{wboxes plot style@\texttt{wboxes} plot
...
modifier@\texttt{using rows} modifier}\index{using columns
modifier@\texttt{using columns} modifier}\label{horizontal_datafiles} The
...
`\texttt{rows}' is placed after the \texttt{using} modifier:\index{rows
keyword@\texttt{rows} keyword}
...
another:\index{columns keyword@\texttt{columns} keyword}
...
labelling it ``\texttt{invisible}''\index{invisible keyword@\texttt{invisible}
...
\index{colours!setting for datasets} In the \texttt{with} clause of the plot
...
\index{set backup command@\texttt{set backup}
