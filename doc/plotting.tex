...
When using the {\tt points}, {\tt linespoints} and {\tt dots} plot
...
{\tt linewidth}\index{linewidth modifier@{\tt linewidth} modifier}
modifier, and the width of the lines used to draw point symbols can be
controlled with the {\tt pointlinewidth}\index{pointlinewidth
modifier@{\tt pointlinewidth} modifier} modifier. For example:
...
{\tt arrows} plot style\index{arrows plot style@{\tt arrows} plot style}.
...
\index{set boxfrom command@{\tt set boxfrom} command}

\subsection{Basic Operation}

As in gnuplot, bar charts and histograms can be produced using the
{\tt boxes}\index{boxes plot style@{\tt boxes} plot style} plot style:
...
widths equalling that specified in the {\tt set boxwidth}\index{set boxwidth
command@{\tt set boxwidth} command} command.  Consequently, there may be
...
behaviour, the {\tt wboxes}\index{wboxes plot style@{\tt wboxes} plot
style} plot style should be used:
...
The bars may be filled using the {\tt with fillcolour}\index{fillcolour
modifier@{\tt fillcolour} modifier} modifier, followed by the name of a
colour:
...
modifier, {\tt switch}\index{select keyword@{\tt select} keyword}. This can be used to
...
modifier@{\tt using rows} modifier}\index{using columns
modifier@{\tt using columns} modifier}\label{horizontal_datafiles} The
...
`{\tt rows}' is placed after the {\tt using} modifier:\index{rows
keyword@{\tt rows} keyword}
...
However, the line-numbers specified in the {\tt every} modifier -- i.e.
...
modifier@{\tt axes} modifier}\index{set axis command@{\tt set axis}
command}
...
using the {\tt set xrange} command\index{set xrange command@{\tt set
xrange} command}, or within the {\tt plot} command. The following two
...
xtics}\index{set xtics command@{\tt set xtics} command} command. The
appearance of ticks along any axis can be turned off with the {\tt set
noxtics}\index{set noxtics command@{\tt set noxtics} command} command. The
...
modifier\index{title modifier@{\tt title} modifier}, as follows:
...
{\tt set key}\index{set key@{\tt set key} command} command.
...
keycolumns} command.\index{set keycolumns command@{\tt set keycolumns}
...
\index{colours!setting the palette}\index{set palette command@{\tt set
...
\index{set backup command@{\tt set backup}
