...
%Gnuplot allows data to be interpolated using its {\tt csplines} plot style,
%for example:\index{csplines plot style@{\tt csplines} plot
%style}\index{acsplines plot style@{\tt acsplines} plot style}
%
%\begin{verbatim}
...
%the new, more powerful, {\tt spline} command\index{spline
%command@{\tt spline} command}. This has a syntax similar to that of the
%{\tt fit} command, for example:
The {\tt spline} command\index{spline command@{\tt spline} command} fits a
...
gnuplot {\tt acsplines} plot style.

\section{Tabulating Functions and Slicing Data Files}

PyXPlot's {\tt tabulate}\indcmd{tabulate} command can be used to produce a text
file containing the values of a function at a set of points.  For example, the
following would produce a data file called {\tt sine.dat} containing a list of
...
differentiation of expressions: {\tt int\_dx()} and
{\tt diff\_dx()}\index{differentiation}\index{integration}\index{int\_dx()
function@{\tt int\_dx()} function}\index{diff\_dx()
function@{\tt diff\_dx()} function}. In each case, the ``{\tt x}'' may be
...
The {\tt histogram}\index{histogram command@{\tt histogram} command}
